...
We need to tune $h$ in OPCG (and MADE):
  ...
  - But Cross Validation can be computationally intensive and costly

  - Can choose $h$ to be the optimal bandwidth $h^{opt} = cn^{-\frac{1}{p_{opt}}}$, but choosing the proportional constant $c$ can be difficult


For classification problems, we propose using a K-means clustering approach. This does not require us to select a prediction method.  
...
1. Estimate $\hat B_{opcg}$ and construct the sufficient predictors $\hat B_{opcg}^\top X$.

2. Apply K-means to $\hat B_{opcg}^\top X$ to get estimated clusters and corresponding F-ratio, i.e. the Within Sums-of-Squares (WSS) over Between Sums-of-Squares (BSS)

3. Select $h$ that minimizes the F-ratio from K-means.

--

But: We ought to incorporate the class/label information available.
...
Improved idea: For each $h$, 

1. For each class/label, 

  a. Estimate $\hat B_{opcg}$ and construct the sufficient predictors $\hat B_{opcg}^\top X$.

  b. Apply K-means to $\hat B_{opcg}^\top X$ to get estimated clusters and corresponding  WSS and  BSS
...
<iframe src="tuning.html" width="100%" height="95%" frameborder="0" ></iframe>
